...
namespace TestSystem.Plot
{
    class Coloring_dotNet : IColoring
    {
        ...
        public Coloring_dotNet(double norm) 
        {
            this.norm = norm;
            if (frmd != null)
        ...
        public Coloring_dotNet(Form_Draw frmd, double norm)
        {
            this.frmd = frmd;
            this.norm = norm;
            frmd.Mult_color(mult);
        ...
        public void ColoringSurface(List<IPoint> points)
        {
            ...
            if (points.Count > 0)
            {
                ...
                else if (points.Count == 2)
                {
                    ...
                    LinearGradientBrush linGrBrush = new LinearGradientBrush(new Point((int)p[0].x1, (int)p[0].x2), new Point((int)p[1].x1, (int)p[1].x2), CostToColor(p[0].cost), CostToColor(p[1].cost));

                    Pen pen = new Pen(linGrBrush);

                    Graphics g = frmd.GetImage();

                    g.DrawLine(pen, (int)p[0].x1, (int)p[0].x2, (int)p[1].x1, (int)p[1].x2);
                }
                else if (points.Count > 2)
                {
        ...
        private Color CostToColor(double cost)
        {
            ...
            if (cost < 256 * 256 * 256)
                return Color.FromArgb(0, (int)(cost * 255 / norm), 0);
            else if (cost < 0)
